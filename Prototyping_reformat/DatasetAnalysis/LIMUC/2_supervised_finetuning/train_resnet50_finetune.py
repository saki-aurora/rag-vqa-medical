#!/usr/bin/env python3
"""Train/evaluate ResNet50 finetune for LIMUC Mayo severity classification."""
...
def _ensure_lzma_module() -> None:
    ...
    Torchvision imports `lzma` in dataset utilities during module import.
    We do not use those code paths for LIMUC training, so a minimal shim is enough.
    """
    ...
    # Constants commonly exposed by stdlib lzma.
    for name in [
        "FORMAT_AUTO",
        "FORMAT_XZ",
        "FORMAT_ALONE",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional metadata_enriched.csv path. Defaults to limuc-root/0_dataset_prep/out/metadata/metadata_enriched.csv",
    )
    parser.add_argument(
        "--label-map-csv",
        type=Path,
        default=None,
        help="Optional label_map.csv path. Defaults to limuc-root/0_dataset_prep/out/metadata/label_map.csv",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=None,
        help="Output run directory. Defaults to limuc-root/2_supervised_finetuning/results/finetune_resnet50_seed<seed>",
    )
    parser.add_argument("--run-id", type=str, default=None, help="Optional run_id override in run_meta.json.")
    ...
    parser.add_argument("--max-samples", type=int, default=0, help="Optional cap over total rows for debugging.")
    parser.add_argument("--device", type=str, default="auto", choices=["auto", "cpu", "cuda"])
    parser.add_argument("--amp", action="store_true", help="Enable automatic mixed precision training.")
    parser.add_argument("--log-every", type=int, default=50, help="Batch logging interval.")
...
def main() -> None:
    ...
    meta_csv = args.meta_csv.resolve() if args.meta_csv else limuc_root / "0_dataset_prep" / "out" / "metadata" / "metadata_enriched.csv"
    label_map_csv = args.label_map_csv.resolve() if args.label_map_csv else limuc_root / "0_dataset_prep" / "out" / "metadata" / "label_map.csv"
    ...
    out_dir = (
        ...
        if args.out_dir
        else (limuc_root / "2_supervised_finetuning" / "results" / f"finetune_resnet50_seed{args.seed:03d}")
    )
    ...
    print(f"[train_resnet50] limuc_root={limuc_root}")
    print(f"[train_resnet50] out_dir={out_dir}")
    print(f"[train_resnet50] device={device}")
    print(
        f"[train_resnet50] seed={args.seed} epochs={args.epochs} batch_size={args.batch_size} "
        f"lr={args.lr} weight_decay={args.weight_decay} amp={args.amp}"
    )
    ...
    if removed > 0:
        print(f"[train_resnet50][warn] dropped {removed} rows with unmapped/invalid labels")
    ...
    print(f"[train_resnet50] split sizes train={len(train_df)} val={len(val_df)} test={len(test_df)}")
    ...
    if n_classes != 4:
        print(f"[train_resnet50][warn] expected 4 classes, found {n_classes}")
    ...
    best_model_path = out_dir / "best_resnet50.pt"

    for epoch in range(1, args.epochs + 1):
        print(f"[train_resnet50] epoch={epoch}/{args.epochs}")
        train_loss, y_train, p_train, prob_train = run_epoch(
        ...
        print(
            f"[train_resnet50] epoch={epoch} train_loss={train_loss:.5f} "
        ...
        if float(val_summary["macro_f1"]) > best_val_macro_f1:
            ...
            print(f"[train_resnet50] new best model saved: val_macro_f1={best_val_macro_f1:.5f}")
    ...
    print("[train_resnet50] complete")
    print(f"[train_resnet50] run_id={run_id}")
    print(f"[train_resnet50] out_dir={out_dir}")
    print(f"[train_resnet50] test_summary={json.dumps(test_summary, indent=2)}")
